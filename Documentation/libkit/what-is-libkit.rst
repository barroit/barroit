...
every repository. It provides general-purpose ``functions`` and ``slots``.

``functions`` are well-defined shell builtins. You can use them out of the box.

``slots`` are also functions, but left undefined. You must define these
yourself to serve as a 'plug'. Otherwise, some builtins won't work.

The core of libkit lives in ``posix/libkit.sh``. Stuff in this file is
POSIX-compliant. But compatibility here only means the syntax follows POSIX. If
a function needs external programs, like ``perl``, you'll see that in its doc.
